...
public class fireVision extends EZPlugin {

  @Command(aliases = { "f" },
          description = "Explode your target into a ball of flaming fire",
          permissions = { "" },
          toolTip = "/f")
  public void firevisionCommand(MessageReceiver caller, String[] args) {
    if (caller instanceof Player) {
      ...
      while (sightItr.hasNext()) {
        ...
        if (b.getType() != BlockType.Air) {
          b.getWorld().setBlockAt(b.getLocation(), BlockType.FireBlock);
          playSound(b.getLocation(), SoundEffect.Type.FIRE_IGNITE);
          playSound(b.getLocation(), SoundEffect.Type.AMBIENCE_THUNDER);
          break;
        }
      }
    }
